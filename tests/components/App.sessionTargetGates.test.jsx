...
it('App passes sessionTarget to every P1 Flow gate and uses canonical route IPC', () => {
  ...
  expect((source.match(/runOuterStartAuthPreflight\(\{[\s\S]{0,180}?sessionTarget[\s\S]{0,180}?\}\)/g) || []))
    .toHaveLength(2)
  expect(source).toContain('window.electronAPI.setRoute(next)')
  expect(source).not.toContain('window.electronAPI?.setMode?.({ mode })')
})
