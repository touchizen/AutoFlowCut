...
const pipeline = (over = {}) => ({
  ...
  scenes: [{ storyId: 'a', segments: [
    { id: 's1', speaker: 'narrator', text: '누구세요?', emotion: 'sad', status: 'done', audioPath: '/a/s1.mp3', startMs: 0, durationMs: 1000, type: 'narration' },
...
describe('세그먼트 감정 2줄 표시', () => {
  it('씬분리 탭: 대화 + (감정) 2줄', () => {
    ...
    expect(within(table).getByText('누구세요?')).toBeTruthy()
    expect(within(table).getByText('(슬픔)')).toBeTruthy()
  ...
  it('오디오 탭: 대화 + (감정) 2줄', () => {
    ...
    expect(within(panel).getByText('누구세요?')).toBeTruthy()
    expect(within(panel).getByText('(슬픔)')).toBeTruthy()
  })

  it('emotion 없으면 기본 (평범) 표시', () => {
    const p = pipeline()
    p.scenes[0].segments[0].emotion = undefined
