...
describe('source 헬퍼 — assert/getVideoDisabledField/buildFramePairVideoPatch (typo 방어)', () => {
  ...
  it('#R33-2: buildFramePairVideoPatch forwards generationId (present-key, incl. explicit null)', () => {
    expect(buildFramePairVideoPatch({ video: 'B', generationId: 'g9' }))
      .toMatchObject({ generationId: 'g9' })
    // restore clears stale routing meta → explicit null must pass through
    expect(buildFramePairVideoPatch({ video: 'B', generationId: null }))
      .toMatchObject({ generationId: null })
    // absent → not added (no spurious clobber)
    expect(buildFramePairVideoPatch({ video: 'B' })).not.toHaveProperty('generationId')
  })
  ...
  it('#R30-4: regenTargetVideoId maps scene-media ids to generation items', () => {
    expect(regenTargetVideoId('t2v_5')).toBe('vscene_5')   // scene-media T2V → video scene
    expect(regenTargetVideoId('i2v_3')).toBe('fp_3')        // scene-media I2V → frame pair
    expect(regenTargetVideoId('vscene_7')).toBe('vscene_7') // already a generation item → unchanged
    expect(regenTargetVideoId('fp_2')).toBe('fp_2')
    expect(regenTargetVideoId(null)).toBeNull()
  })
})
