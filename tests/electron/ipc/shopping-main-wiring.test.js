...
describe('shopping IPC main wiring', () => {
  it('registers Story and Shopping with one main-owned coordinator and canonical work-folder authority', () => {
    ...
    expect(source).toMatch(/registerShoppingIPC\(ipcMain, \{[\s\S]*?getWindow: \(\) => mainWindow,[\s\S]*?getActiveWorkFolder: \(\) => workFolderAuthority\.getVerifiedContext\(\),[\s\S]*?workflowSessions,[\s\S]*?\}\)/)
