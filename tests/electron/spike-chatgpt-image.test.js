...
describe('saveImage', () => {
  it('fetches through the view session (partition cookies) and writes the bytes', async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(CDN, { credentials: 'include' })
  ...
  it('rejects a 200 non-image response before creating the output directory', async () => {
    const { view } = makeView({ contentType: 'text/html; charset=utf-8', bytes: [60, 33, 68, 79, 67] })
    const fs = makeFs()
    await expect(saveImage(app, view, CDN, fs, {})).rejects.toThrow(/content-type.*text\/html/i)
