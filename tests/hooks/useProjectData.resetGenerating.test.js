...
describe('resetGeneratingItem', () => {
  ...
  it('videoI2VStatus=generating → pending, videoI2VGeneratingStartedAt 비움, videoI2VPath 보존(복귀)', () => {
    const out = resetGeneratingItem({ videoI2VStatus: 'generating', videoI2VPath: '/i.mp4', videoI2VGeneratingStartedAt: 7 })
    expect(out.videoI2VStatus).toBe('pending')
    expect(out.videoI2VGeneratingStartedAt).toBe(undefined)
