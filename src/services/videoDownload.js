 ...
 *
 * @param {Function} downloadVideo - useGenAPI.downloadVideo(videoUri, resolution, generationId) → {success, base64, mimeType, error}
 * @param {string} videoUrl - 완료된 Veo operation 의 video.uri
 * @param {string} [generationId] - provider 를 식별하는 generation handle
 * @returns {Promise<{success:boolean, base64?:string, mimeType?:string, error?:string}>}
 */
export async function downloadVideoBase64(downloadVideo, videoUrl, resolution = undefined, generationId = undefined) {
  ...
  try {
    ...
    return generationId != null
      ? await downloadVideo(videoUrl, resolution, generationId)
      : await downloadVideo(videoUrl, resolution)
  } catch (e) {
