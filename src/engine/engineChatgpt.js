...
// C1 (wave 2: aspectRatio, wave 3: seed): every scene/reference caller forwards the
// project-level aspectRatio ('16:9' default — ALWAYS a string) and seed (seedLocked:true +
// numeric seedNo default — a NUMBER out of the box) unconditionally. That is settings
// plumbing, not an explicit per-request opt-in — refusing either killed every real
// generation. The engine drops both from the IPC payload instead and fires ONE notice so
// the app can surface once per session what this target cannot control: aspect ratio
// (spike measured square output only) and seed (never measured — no fabricated support).
const defaultUnmeasuredOptionsNotice = () => {
  try {
    globalThis.window?.dispatchEvent?.(new CustomEvent('chatgpt-unmeasured-options-ignored'))
...
export function createChatgptEngine({
  ...
  onUnmeasuredOptionsIgnored = defaultUnmeasuredOptionsNotice,
} = {}) {
  const submitGeneration = async (prompt, referenceImages = [], options = {}) => {
    ...
    // batchCount's default (1) passes, so this refusal fires only on explicit user opt-in.
    if (options.batchCount != null && options.batchCount !== 1) {
      return optionRefusal('chatgpt-batch-count-unmeasured', 'batch count')
    }
    // aspectRatio/seed arrive on EVERY default run — dropped (with one notice), never refused.
    if (options.aspectRatio != null || options.seed != null) onUnmeasuredOptionsIgnored()
    if (typeof electronAPI?.chatgptSubmitGeneration !== 'function') return unavailable('generation')
    // aspectRatio/seed/provider/model are intentionally omitted — unmeasured on this target.
