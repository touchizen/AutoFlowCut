...
### 내보내기 (CapCut / Premiere Pro / Vrew)
- **멀티 에디터 내보내기** — CapCut, Adobe Premiere Pro(`.prproj`), Vrew(`.vrew`)로 원클릭 내보내기
- **완성 프로젝트** — 타임라인, 미디어, 자막, Ken Burns 애니메이션 포함
- **프로젝트 파일 직접 쓰기** — 디스크에 바로 기록 (수동 ZIP/임포트 불필요)
- **자동 실행 + 설치 안내** — 내보내기 후 대상 에디터 자동 실행. 미설치 시 다운로드(또는 스토어) 페이지 안내
...
- **Story Engine v2** — 9-Wave 자동 파이프라인으로 대본부터 내보내기까지 전자동
...
AutoFlowCut에 대한 커뮤니티 기여에 적극적으로 보상합니다.

### 🐞 버그 수정 → 사용 크레딧

앱의 버그를 찾아 수정하고 PR을 보내주세요. 리뷰 후 머지되면 기여도 평가에 따라
**사용 크레딧**을 드립니다 — **최소 10회 생성**부터 **1년 무제한 사용**까지.

### 🔌 수익모델 플러그인 → 머지

수익 모델을 더하는 플러그인을 개발하셨나요? PR을 보내주세요. 프로젝트에 맞고
안전하게 만들어진 플러그인은 **리뷰 후 머지**하고 세부 사항을 함께 조율합니다.

PR을 열거나 [Discord](https://discord.gg/DTMMs8TZDN)에서 논의를 시작하세요. 모든
기여는 AGPL v3 로 받습니다 — [CONTRIBUTING.md](CONTRIBUTING.md) 참고. 보상 규모는
각 기여의 품질과 영향도에 따라 메인테이너 재량으로 지급됩니다.
