...
- [ ] **TTS / SFX API key 사전 검증 (W5-0-prep preflight)**: 현재 ElevenLabs / Typecast 키는 lazy load라 W5 중간에 ENOENT / 401로 실패 — 5~10분 작업 손실. Google AI Studio (Gemini) 지원은 spec에 "TBD"로만 있고 `readApiKey` 미구현. 해결: W5-0-prep에서 provider 선택 직후 필요한 키 셋만 cheap GET (`/v1/voices`, `models?key=`)으로 검증, 누락/실패 시 AskUserQuestion으로 paste/open-file/switch-provider 옵션 제공. 페이스트한 키는 `~/.<provider>/credentials`에 mode 0600 dotenv로 영속화. `readApiKey`에 `gemini` 추가. 상세 계획: [docs/plans/tts-api-key-preflight.md](docs/plans/tts-api-key-preflight.md). Estimated ~3시간.
