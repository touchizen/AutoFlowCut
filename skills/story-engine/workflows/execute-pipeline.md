...
| W6   | `final_{part}.srt`, **`timeline_{part}.json`**, `narration_{part}.txt`, `{title}_*.md` (script), `08_sfx_목록.md`, **`voices/result.json`** + **`voices/*_{HHMMSS}.mp3`** (when dialogue — primary speaker→timecode source for `splitOnSpeakerChange: true`; carries TTS-resolved start/duration including consecutive-dialogue stacking), `dialogs_{part}.json` + `segments_{part}/index.json` (fallback anchor for single-dialog-per-paragraph cases) | `references.csv`, `{title}_scenes.csv`, `06_review_group{A,B,C}.md` (batch QA)                                |
