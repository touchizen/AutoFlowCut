...
- **MANDATORY W5-0 provider selection + key preflight + voice assignment** (BEFORE TTS):
  ...
  - **5-0-prep key preflight (MANDATORY between provider-pick and voice-assign):**
    ...
    - Gate: proceed to 5-0-assign **only after every provider in the required set has validated 200 OK**. Transient 5xx is logged as "validation skipped, proceed with warning" — do NOT block on infra outages; the actual TTS call will surface a more specific failure if the key is truly bad.
