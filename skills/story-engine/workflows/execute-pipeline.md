...
| W4   | **W4-0 production-scope (user gate — AskUserQuestion if `production_scope` missing in STATE.md; otherwise skip)**, W4-1 read-inputs, W4-2..5 narration-extract × 4, W4-6 dialogue-extract (skip when `production_scope.dialogue: false`), W4-7 SFX-list (skip when `production_scope.sfx: false`), W4-8 audit. **Sub-step IDs here are NOT the same as the doc-section numbers in `docs/{lang}/W4-production.md`**: doc § 4-1 narration ⇄ sub-steps W4-2..5; doc § 4-2 dialogue ⇄ sub-step W4-6; doc § 4-3 SFX ⇄ sub-step W4-7. (Pre-existing offset preserved for back-compat; do not renumber the doc.) |
| W5   | W5-0-prep provider-pick + key-preflight (>3 min OK if user is pasting keys — emit `▸ Waiting for API key entry…` heartbeat every 30–60 s while paused on AskUserQuestion or `(d) retry later`), W5-0-assign voice-assign (narrator-only when `production_scope.dialogue: false`), W5-1a narration-TTS, W5-1b draft-subs, W5-1c build-SRT+timeline, W5-1d user-review (optional), W5-1e merge-segments, W5-1f dialogue-TTS (skip when `production_scope.dialogue: false` OR `dialogs_{part}.json` absent), W5-2 SFX (skip when `production_scope.sfx: false` OR `08_sfx_*.md` absent), W5-3 4-part merge (SFX timecode conversion no-op when sfx off), W5-4 mechanic-QA (SFX checks vacuous → empty cue list = pass when sfx off) |
...
- **MANDATORY W5-0 provider selection + key preflight + voice assignment** (BEFORE TTS):
  ...
  - **5-0-prep key preflight (MANDATORY between provider-pick and voice-assign):**
    - Compute the required-key set as **{narration provider} ∪ {dialogue provider when `production_scope.dialogue: true`} ∪ {ElevenLabs when `production_scope.sfx: true`}** (Vrew contributes nothing — local app, no key).
    - For each provider in the required set, run `readApiKey(provider)` (env → file fallback; already supported in `lib_afc.cjs` for `elevenlabs` / `typecast`), then issue the cheap GET validation:
      - **ElevenLabs**: `GET https://api.elevenlabs.io/v1/voices` with `xi-api-key: <key>` (200 OK / 401 invalid).
      - **Typecast**: `GET https://api.typecast.ai/v1/voices` with `x-api-key: <key>` (200 OK / 401 invalid).
      - **Gemini** (forward-compat — `readApiKey` does NOT yet support `gemini`; v1 bundle has no Gemini-consuming script): `GET https://generativelanguage.googleapis.com/v1beta/models?key=<key>` (200 OK / 400/403 invalid).
    - Classify each result: Found+OK → continue; Not found → first-time-setup menu; Found+401/403 → stale-key menu. Emit user-facing chat lines per the W5 doc § 5-0-prep + key preflight ("▸ Validating … / ✅ … OK / ⚠ … missing").
    - Remediation menu (AskUserQuestion, same four options for miss + stale): (a) "I have a key — paste into credentials file" — print exact path `~/.<provider>/credentials` + line format `<ENVNAME>=<value>` + chmod 0600 reminder, **v1: user pastes manually; subagent does NOT write the file**; on user confirmation re-run the GET validation; (b) "Show me where to get a key" — print the provider's signup URL verbatim (`https://elevenlabs.io/app/speech-synthesis/api-keys`, `https://app.typecast.ai/api-keys`, `https://aistudio.google.com/app/apikey`), loop back to (a); (c) "Switch to a different provider" — reopen the W5-0-prep AskUserQuestion, recompute the required-key set, restart preflight from Step 2; (d) "I'll set it up myself, retry later" — print path, pause pipeline; on `/story-resume` re-enter the loop at Step 2 for whichever providers were not yet OK.
    - Gate: proceed to 5-0-assign **once every provider in the required set is either 200 OK or transient-5xx (warning, non-blocking)** — no provider may remain in the {miss, 401/403} blocking buckets. Transient 5xx is logged as "validation skipped, proceed with warning" — do NOT block on infra outages; the actual TTS call will surface a more specific failure if the key is truly bad.
    - Heartbeat: while paused on AskUserQuestion for paste/open-file/retry-later, the orchestrator emits `▸ Waiting for API key entry…` every 30–60 s so the user knows the pipeline is waiting on them, not stuck.
    - **`production_scope` interaction (explicit):** `dialogue: false` drops the dialogue-side Typecast requirement (narration-side Typecast still counted if narration = Typecast); `sfx: false` drops ElevenLabs-for-SFX (if no other slot uses ElevenLabs, drop ElevenLabs entirely — do NOT validate it).
