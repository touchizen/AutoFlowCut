...
| W6   | `final_{part}.srt`, **`timeline_{part}.json`**, `narration_{part}.txt`, `{title}_*.md` (script), `08_sfx_목록.md`, `dialogs_{part}.json` (when present — required for `splitOnSpeakerChange: true` speaker→timecode mapping), `segments_{part}/index.json` (paragraph_idx anchor for dialogue placement) | `references.csv`, `{title}_scenes.csv`, `06_review_group{A,B,C}.md` (batch QA)                                |
