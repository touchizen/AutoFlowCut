...
- **`voices/{part}_*_{HHMMSS}.mp3`** (when dialogue exists) — filename pattern is `{part}_{order}_{character}_{HHMMSS}.mp3`; the leading `{part}_` token disambiguates per-part files, and the `HHMMSS` is the per-part resolved start. If `voices/result_{part}.json` is missing, the part, speaker, and per-part start can all be recovered from the filename alone (duration from ffprobe). (Earlier versions wrote `{order}_{character}_{HHMMSS}.mp3` without a part prefix, which could collide across parts — fixed by the prefix.)
...
2. **`voices/{part}_*_{HHMMSS}.mp3` filename parse** — when `result_{part}.json` is absent but the mp3 files exist, the filename `{part}_{order}_{character}_{HHMMSS}.mp3` carries part, speaker, and per-part resolved start; compute duration with ffprobe.
