...
> **`production_scope.sfx: false` 또는 `media/sfx/` 부재 → 본 단계 전체 스킵**. SFX가 생성되지 않은 에피소드는 검증할 대상이 없음. 8-1 (오디오 임포트)으로 즉시 진행. (오디오 임포트는 디스크에 존재하는 오디오만 imports 하므로 SFX 트랙이 없는 채로 정상 동작.)
