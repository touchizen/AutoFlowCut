...
## 8-1. 오디오 임포트 검증 (없으면 first-time 임포트 폴백)

현재 파이프라인에서 **오디오는 보통 W5-3a 시점에 임포트된다** (W5-3가
`media/final_full.mp3` + `media/sfx/`를 생성한 직후). 그래서 사용자는
W6/W7가 도는 동안 Audio 탭에서 TTS를 검토/플래그 가능. W8 시작 시점에는
오디오가 이미 임포트되어 있고 리뷰까지 끝나있는 경우가 많다.

**W8-1 프로토콜:**

1. **기존 임포트 상태 확인** — 에피소드 폴더로 `/api/audio-reviews` GET.
   - 응답이 비어 있지 않으면 (플래그 유무 무관 항목 존재) → 이미 임포트됨, **3번으로**.
   - 응답이 비어 있고 `media/final_full.mp3`가 디스크에 존재 → 오디오 파일은 있는데 임포트가 안 됨 (W5-3a 스킵/실패 또는 spec 이전 에피소드). **2번 first-time 임포트**.
   - `media/final_full.mp3`가 없으면 → 에스컬레이션. W5가 완료되지 않은 상태.
2. **First-time 임포트 (폴백 전용)** — 아래 POST. 기존 W8-1 로직 그대로.
3. **Refresh + 플래그 처리** — `/api/audio-refresh` POST로 재스캔 + 재생성된 파일 자동 unflag. 남은 플래그 항목은 W8-1 리뷰 리포트에 surface.

임포트(또는 검증) 후, CapCut 내보내기 시 나레이션/SFX가 타임라인에 자동 배치된다.
