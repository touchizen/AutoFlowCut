...
> **`production_scope.sfx: false` 또는 `media/sfx/` 부재 → 본 단계 전체 스킵**. SFX가 생성되지 않은 에피소드는 검증할 대상이 없음. 8-1 (오디오 임포트)으로 즉시 진행. (오디오 임포트는 디스크에 존재하는 오디오만 imports 하므로 SFX 트랙이 없는 채로 정상 동작.) 플래그 전체 스펙과 legacy-fallback 규칙은 `workflows/execute-pipeline.md` → "STATE.md schema — production_scope block" 참조.
...
└── media/
    ...
    │   └── 두목/
    │       └── ...
    └── sfx/                  ← SFX (flat — W5가 `{NN}_{filename}_{HHMMSS}.mp3` 형식으로 media/sfx/ 직속에 씀)
        ├── 01_주판_구슬_0030.mp3   ← cue #01, anchor stem `01_주판_구슬`, full-timeline 00:30
        ├── 13_시장_0836.mp3       ← cue #13, 08:36 배치
        └── ...
        # 주의: 파일명은 W4-3의 `{NN}_{filename}` stem 을 보존해야 AudioTab SFX
        # 프롬프트 매칭이 `08_sfx_목록.md` 와 조인 가능. 카테고리 서브폴더로
        # 쪼개지 말 것 (stem 이 깨짐).
```
