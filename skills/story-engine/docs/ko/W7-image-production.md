...
W7은 **이미지 생성**과 **이미지 검수**만 다룬다. 오디오 1차 임포트는 W5-5(W5-4 mechanic QA 통과 후 best-effort)에서 처리되어, 사용자는 W7이 도는 동안 Audio 탭에서 TTS를 병렬로 검토 가능. CapCut export, 영상 생성, audio import idempotent 재호출(W5-5 스킵/실패 시 안전망)은 W8(어셈블리)이 담당한다.
