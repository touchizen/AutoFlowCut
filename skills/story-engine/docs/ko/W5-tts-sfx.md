...
> **스크립트 경로 (모든 5-1 단계 공통):**
>
> 5-1a~f의 모든 명령은 **설치된 스킬 번들**의 스크립트를 호출한다. cwd가 repo 루트가 아니어도 작동하도록 절대 경로를 셸 변수로 한 번 잡는다. **사용 중인 셸**에 맞는 한 줄만 실행하면 그 다음 5-1 명령은 모두 동일.
>
...
>
> 아래 5-1 명령은 모두 bash/PowerShell 형식. **cmd.exe 사용자만** `$SCRIPT_DIR` → `%SCRIPT_DIR%`, `/` → `\`로 치환. Node.js는 Windows에서도 forward slash를 허용하므로 PowerShell은 추가 변환 불필요.
>
...
node "$SCRIPT_DIR/generate_tts_elevenlabs.cjs" \
  ep{N}/narration_{파트}.txt  ep{N}/segments_{파트}/  <narrator_voice_id>
```
...
node "$SCRIPT_DIR/draft_subtitles.cjs" \
  ep{N}/segments_{파트}/  ep{N}/subtitles_{파트}.txt
```
...
node "$SCRIPT_DIR/build_srt.cjs" \
  ep{N}/segments_{파트}/  ep{N}/subtitles_{파트}.txt  ep{N}/final_{파트}.srt \
  ep{N}/timeline_{파트}.json     # ← 4번째 인자 필수 (W6 입력)
```
**산출:** `final_{파트}.srt` (alignment 기반 정확한 타임코드) + `timeline_{파트}.json` (세그먼트별 누적 시작/끝, W6 scenes.csv 빌드 입력)
...
node "$SCRIPT_DIR/merge_audio.cjs" \
  ep{N}/segments_{파트}/  ep{N}/final_{파트}.mp3
```
...
node "$SCRIPT_DIR/generate_tts_typecast.cjs" dialogue \
  ep{N}/dialogs_{파트}.json  ep{N}/voices/  ep{N}/tts_settings.md  \
  ep{N}/segments_{파트}/      # ← 4번째 인자: 5-1a 산출 segments dir
```
