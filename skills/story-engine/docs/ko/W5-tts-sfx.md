...
>
> 5-1a~f의 모든 명령은 **설치된 스킬 번들**의 스크립트를 호출한다. cwd가 repo 루트가 아니어도 작동하도록 절대 경로를 셸 변수로 한 번 잡고 재사용한다.
>
> ```bash
...
> ```
>
> 이후 모든 명령은 `node "$SCRIPT_DIR/<script>.cjs" ...` 형태. repo 루트의 `skills/story-engine/scripts/` 상대 경로는 개발자 모드 외엔 깨지므로 사용 금지.
