...
이 문서는 story-engine 스킬의 W7(이미지/영상 생성 + QA + CapCut 내보내기) 단계 가이드입니다.
...
**리뷰 (서브스텝 7-2b)** — 서브에이전트 자가검토 → 이슈 목록 → 수정. 최대 5회. 0 이슈 시 다음 서브스텝(7-2c)으로 즉시 진행. 5회 초과 시 사용자에게 에스컬레이션.

### 7-2c. 오디오 임포트 (나레이션 + SFX)

이미지 QA 완료 후, W5에서 생성한 오디오 파일을 AutoFlowCut에 임포트한다.
오디오 임포트 후 CapCut 내보내기 시 나레이션/SFX가 타임라인에 자동 배치된다.

**오디오 임포트 전 SFX 씬 매칭 검증 (필수, W5에서 이전됨):**

W5-4에서는 `scenes.csv` (W6 산출물)에 의존하는 검증을 수행할 수 없으므로, 오디오 임포트 직전인 이 시점에 한 번 더 검증한다.

```python
# 1. scenes.csv에서 씬별 start_time / end_time / parent_scene 읽기
# 2. 08_sfx_목록.md 또는 W4 SFX 목록의 "위치" → parent_scene 매핑
# 3. media/sfx/ 파일명의 타임코드(MMSS/HHMMSS)와 비교
# 4. 각 SFX의 타임코드가 매핑된 씬의 [start_time, end_time] 범위 안에 있는지 확인
# 5. 불일치 시 fix_sfx_timecodes.py로 재매핑 → 다시 검증 → pass까지 반복
```

**검증 항목:**
- SFX 타임코드 ∈ [매핑 씬의 start_time, end_time] (±0.5초 허용)
- 어떤 SFX도 씬 경계 밖으로 떨어지지 않음
- 모든 `media/sfx/*.mp3` 파일이 씬에 매핑됨 (orphan SFX 없음)

**STATE.md 업데이트:**
- step: `W07_sfx_scene_match_qa`
- 씬 매칭 검증 통과 후 기록

검증 통과 후에만 아래 임포트 절차로 진행한다.
...
**리뷰 (서브스텝 7-2c)** — 서브에이전트 자가검토 → 이슈 목록 → 수정. 최대 5회. 0 이슈 시 다음 서브스텝(7-2d)으로 즉시 진행. 5회 초과 시 사용자에게 에스컬레이션.
...
**리뷰 (서브스텝 7-2d)** — 서브에이전트 자가검토 → 이슈 목록 → 수정. 최대 5회. 0 이슈 시 다음 서브스텝(7-3)으로 즉시 진행. 5회 초과 시 사용자에게 에스컬레이션.
...
**리뷰 (서브스텝 7-3)** — 서브에이전트 자가검토 → 이슈 목록 → 수정. 최대 5회. 0 이슈 시 다음 Wave로 즉시 진행. 5회 초과 시 사용자에게 에스컬레이션.

---

## Wave 리뷰 요약
위 각 서브스텝은 최대 5회 리뷰(0 이슈 시 즉시 진행)를 강제한다. 마지막 서브스텝의 리뷰가 통과하면 Wave 7 완료. 어느 서브스텝이든 5회 초과 시 사용자에게 에스컬레이션.
