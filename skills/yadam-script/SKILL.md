...
> **모든 단계 전환 시 반드시 `./check_step.sh {다음단계}` 게이트 검증을 실행한다. 예외 없음.**

**진행 상태 파일**: 에피소드 디렉토리의 `R_progress.json`
- 단계 완료 시 status를 `"done"` + `completed_at` 기록
- 게이트 차단 시 선행 단계를 먼저 완료 (절대 건너뛰지 않음)

**자동/수동 구분:**
- **R1~R6**: 자동 진행 (사용자 확인 없이 게이트만 통과하면 다음 단계로)
- **R7 (대본 확정)**: 🛑 사용자 확인 필수
- **R8~R12**: 자동 진행 (게이트만 통과하면 다음 단계로)
...
**🔴 레퍼런스 이미지 수정 시 절대 규칙:**
- **잘못된 카드의 이미지만 개별 삭제(`app_clear_reference_image`)하고 개별 재생성(`app_generate_reference`)한다**
- **절대 전체 이미지를 일괄 삭제(`app_clear_all_reference_images`)하지 않는다** — 정상 이미지까지 날리면 시간/비용 낭비
- 프롬프트 수정이 필요한 카드만 `app_update_reference`로 수정 후 해당 카드만 재생성
