...
- `sceneId` (필수): 씬 ID
- `styleId` (선택): 스타일 ID. 형식은 `app_generate_reference`와 동일. 생략 시 기존 동작 (style_tag fallback만).

> 이미 완료된 씬도 다시 호출하면 새 styleId로 재생성됩니다.

#### POST /api/start-scene-batch

```json
{ "styleId": "preset:korean-ani", "force": true }
```

- `styleId` (선택): 스타일 ID. `auto` (씬별 매칭) / `none` (스타일 미적용 강제) / 명시 id 등.
- `force` (선택, 기본 false): true면 완료된 씬도 포함해 모든 씬을 새 styleId로 강제 재생성.

Body 없이 POST하면 첫 style 카드 자동 fallback. 앱의 "생성 시작" 버튼과 동일.

> 진행 중에 호출하면 자동으로 현재 batch를 중지하고 새 스타일로 재시작합니다 (확인 모달 없음).
...
- `styleId` (선택): 스타일 ID. `none` (미적용 강제) / 명시 id 등. `"auto"`는 ref 컨텍스트에서 의미 없음 — 보내도 무시되고 fallback 동작.
- `force` (선택, 기본 false): true면 완료된 reference도 포함해 모든 ref를 새 styleId로 강제 재생성.

Body 없이 POST하면 첫 style 카드 자동 fallback.

> 진행 중에 호출하면 자동으로 현재 batch를 중지하고 새 스타일로 재시작합니다.
...
| `app_generate_scene` | 개별 씬 이미지 생성 | `sceneId`, `styleId?` |
| `app_start_scene_batch` | 씬 일괄 생성 시작 | `styleId?`, `force?` |
| `app_start_ref_batch` | 레퍼런스 일괄 생성 시작 | `styleId?`, `force?` |
...
- `styleId` (optional): Style ID. Same format as `app_generate_reference`. Omit for existing behavior (style_tag fallback only).

> Calling this on an already-completed scene re-generates it with the new styleId.
...
- `styleId` (optional): Style ID. `auto` (per-scene matching) / `none` (force no style) / explicit id.
- `force` (optional, default false): If true, re-generate completed scenes too — apply new styleId to all scenes.

Empty body → first style card auto-fallback. Equivalent to clicking "Start Generation" in the app.

> Calling this while a batch is running automatically stops the current batch and restarts with the new style (no confirmation modal).
...
- `styleId` (optional): Style ID. `none` (force no style) / explicit id. `"auto"` is meaningless for refs (silently ignored).
- `force` (optional, default false): If true, re-generate completed references too — apply new styleId to all refs.

Empty body → first style card auto-fallback.

> Calling this while a batch is running automatically stops the current batch and restarts with the new style.
...
| `app_generate_scene` | Generate a single scene image | `sceneId`, `styleId?` |
| `app_start_scene_batch` | Start scene batch generation | `styleId?`, `force?` |
| `app_start_ref_batch` | Start reference batch generation | `styleId?`, `force?` |
