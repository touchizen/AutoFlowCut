...
export function registerGenaiIPC(ipcMain, deps) {
  ...
  ipcMain.handle('genai:generate-video', async (_e, params = {}) => {
    const apiKey = keyStore.getKey()
    if (!apiKey) return { success: false, error: 'No API key' }
    const { prompt, image = null, endImage = null, aspectRatio, durationSeconds, model, seed, resolution } = params
    const res = await submitVideo({ apiKey, prompt, image, endImage, aspectRatio, durationSeconds, model, seed, resolution }, engineDeps)
