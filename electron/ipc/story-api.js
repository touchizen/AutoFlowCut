...
export function registerStoryIPC(ipcMain, { keyStore, getWindow, llm = llmGemini, loadMetaPrompt, getActiveWorkFolder = () => null, tts, ttsFor, probe, defaultVoice, sfxFor, youtube, factCheck, listClaudeModels = llmClaude.listClaudeModels, listCodexModels = defaultListCodexModels, resolveKeyWithSource, safeStorage, workflowSessions = createWorkflowSessionCoordinator() }) {
  ...
  const guarded = (fn) => async (_e, payload = {}) => {
    const session = workflowSessions.capture('story', payload.projectToken)
    if (!session) return { error: 'stale-token' }
    return fn(payload, session.machine)
  }
  ...
  ipcMain.handle('story:load-audio-package', async (_e, { projectPath } = {}) => {
    ...
    if (projectPath) {
      ...
      return asKind(() => readAudioPackage(context.projectPath))
    }
    const session = workflowSessions.current('story')
    if (!session) return null
    return asKind(() => session.machine.loadAudioPackage())
  })
  ...
  ipcMain.handle('story:audio-preflight', async (_e, params) => {
    ...
    const session = workflowSessions.current('story')
    if (!session) return buildAudioPreflightResult([], { resolveKeyWithSource, encryptionAvailable })
    const required = await session.machine.audioPreflight(params || {})
    return buildAudioPreflightResult(required, { resolveKeyWithSource, encryptionAvailable })
  })

  ipcMain.handle('story:generate-title', guarded(({ scriptMd, options }, currentMachine) => currentMachine.generateTitle(scriptMd, options || {})))
  ...
  ipcMain.handle('story:generate-synopsis', guarded(({ type, title, pastedScript, options, useResearch }, currentMachine) =>
    currentMachine.generateSynopsis({ type, title, pastedScript, useResearch, options: options || {} })))
  // 시놉시스 검수(spec 2026-07-10): draft-only — 결과를 돌려줄 뿐 저장하지 않는다.
  ipcMain.handle('story:review-synopsis', guarded(({ synopsisMd, characters, options, review }, currentMachine) =>
    currentMachine.reviewSynopsis({ synopsisMd, characters, options: options || {}, review })))
  ...
  ipcMain.handle('story:confirm-synopsis', guarded(({ synopsisMd, characters }, currentMachine) =>
    currentMachine.confirmSynopsis({ synopsisMd, characters })))
  ...
  ipcMain.handle('story:tts-preview', guarded(async ({ segmentIds, speakers, sfxSources }, currentMachine) => {
    try {
      return await currentMachine.synthPreview({ segmentIds, speakers, sfxSources })
    } catch (e) {
      if (!e?.errorKind) throw e
  ...
  ipcMain.handle('story:research-search', guarded(({ query, keyword, maxResults, dateFilter }, currentMachine) =>
    currentMachine.researchSearch({
  ...
  ipcMain.handle('story:research-fetch', guarded(({ videoIds, options }, currentMachine) =>
    currentMachine.researchFetchTranscripts({ videoIds: videoIds || [], options: options || {} })))
  ipcMain.handle('story:research-analyze', guarded(({ videoIds, options }, currentMachine) =>
    currentMachine.researchAnalyze({ videoIds, options: options || {} })))
  ipcMain.handle('story:research-factcheck', guarded(({ options }, currentMachine) =>
    currentMachine.researchFactCheck({ options: options || {} })))
  // 개선4/m3: adoptedIndices — 팩트체크 주장의 수동 채택 인덱스 목록(미전달 시 supported만).
  ipcMain.handle('story:research-commit', guarded(({ analysis, verifiedClaims, adoptedIndices }, currentMachine) =>
    currentMachine.researchCommit({ analysis, verifiedClaims, adoptedIndices })))
  ipcMain.handle('story:research-skip', guarded((_payload, currentMachine) => currentMachine.researchSkip()))
  // m5: 수동 URL 카드·fetch 전 선택 영속(draft) — 탭전환/재오픈 유실 방지.
  ipcMain.handle('story:research-select', guarded(({ selectedVideoIds, manualVideos }, currentMachine) =>
    currentMachine.researchSelect({ selectedVideoIds, manualVideos })))
  // 상세 모달(2026-07-08): 영상 카드 더블클릭 시 단일 영상 상세 조회(구독자·게시일·바이럴 지수).
  // 파이프라인 상태 불변 — 온디맨드 읽기 전용.
  ipcMain.handle('story:research-video-details', guarded(({ videoId }, currentMachine) =>
    currentMachine.researchVideoDetails({ videoId })))
}
