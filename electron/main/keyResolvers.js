...
export function buildKeyResolvers({ multiKeyStore, genaiKeyStore, getTypecastKey, readCredentialsKey, disableFallback }) {
  ...
  // provider 목록·별칭·store 분류는 registry에서만 가져와 main 쪽 allowlist 드리프트를 막는다.
  const providers = Object.keys(API_KEY_REGISTRY)
  const storeByKeyId = Object.fromEntries(providers.map((provider) => [
    keyIdForProvider(provider), storeForProvider(provider),
  ]))
  const FALLBACK = {
  ...
  function resolveKeyWithSource(keyId) {
    const storeType = storeByKeyId[keyId]
    if (storeType === 'genai') {
      const k = genaiKeyStore.getKey() ?? null
      return k ? { key: k, source: 'store' } : { key: null, source: null }
    }
    const stored = storeType === 'multi' ? (multiKeyStore.getKey(keyId) || null) : null
  ...
  const ttsKeyFor = Object.fromEntries(providers.map((provider) => {
    const keyId = keyIdForProvider(provider)
    return [provider, () => resolveKeyWithSource(keyId).key]
  }))
  const sfxKeyFor = {
