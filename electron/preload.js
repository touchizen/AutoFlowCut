...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  stageImageFirstImage: (params) => ipcRenderer.invoke('fs:stage-image-first-image', params),
  abortImageFirstImport: (params) => ipcRenderer.invoke('fs:abort-image-first-import', params),
  commitImageFirstImport: (params) => ipcRenderer.invoke('fs:commit-image-first-import', params),
  ...
  onAgentEvent: (channel, cb) => {
    ...
    const valid = ['agent:delta', 'agent:message', 'agent:tool-call', 'agent:item-retracted', 'agent:usage', 'agent:done', 'agent:error']
    if (!valid.includes(channel)) return () => {}
    const listener = (_e, payload) => cb(payload)
    ipcRenderer.on(channel, listener)
    return () => ipcRenderer.removeListener(channel, listener)
