# AutoFlowCut — Microsoft Store Listing
...
Generate 100+ images. Create T2V and I2V videos. Export a complete CapCut project. All in one desktop app.

AutoFlowCut automates the entire AI video creation pipeline — from Google Flow AI image/video generation to a ready-to-edit CapCut project. Import your script, generate visuals, select the best media per scene, and export everything with one click.
...
3. Generate images — Batch-create 100+ AI images with Google Flow AI. Auto-retry on errors.
...
6. Export to CapCut — One click exports a complete project with timeline, media, subtitles, and Ken Burns animations.
...
5. Select export media — For each scene, choose image, T2V video, or I2V video. Or let auto-mode pick the best available.
6. Export to CapCut — Generates a complete CapCut project. Open in CapCut and start editing immediately.
...
- One-Click CapCut Export — Timeline, media files, subtitles, and Ken Burns animations in one project file.
- Style Presets — Choose from 80+ built-in style presets (anime, watercolor, cinematic, etc.) to apply consistent visual styles across all scenes. Optional "Require Style" setting ensures a style is always selected before generation.
...
- MCP Server (Claude Code) — Built-in MCP server lets Claude Code edit scenes, prompts, references, and trigger generation directly. Automate your entire video production workflow with AI coding assistants.
- Open Source — Free forever. View and contribute on GitHub.


🤖 CLAUDE CODE INTEGRATION

AutoFlowCut includes a built-in MCP (Model Context Protocol) server that connects directly to Claude Code. This means you can use AI coding assistants to automate your video production workflow:
...
- AI Story Channels — Keep characters, backgrounds, and styles consistent across 200+ scenes.
- Shorts & TikTok Creators — Quickly turn AI-generated scenes into short-form video projects.
- Educators & Course Creators — Turn scripts into illustrated video lessons with AI visuals.
...
### Keywords
```
AI video, CapCut, Google Flow, text to video, image to video, AI image generator, video automation, batch generation, Ken Burns, subtitle, SRT, faceless YouTube, AI storytelling, open source
```
...
이미지 100장 이상 생성. T2V, I2V 비디오 생성. CapCut 프로젝트 원클릭 내보내기. 하나의 데스크톱 앱으로.

AutoFlowCut은 AI 영상 제작 전 과정을 자동화합니다 — Google Flow AI로 이미지/비디오를 생성하고, 바로 편집 가능한 CapCut 프로젝트로 변환합니다. 대본을 가져오고, 비주얼을 생성하고, 씬별로 최적의 미디어를 선택하고, 원클릭으로 내보내세요.
...
3. 이미지 생성 — Google Flow AI로 100장 이상의 AI 이미지를 일괄 생성. 에러 자동 재시도.
4. 비디오 생성 — 선택한 씬에 T2V(텍스트→비디오) 또는 I2V(이미지→비디오) 생성.
5. 미디어 선택 — 씬별로 이미지, T2V, I2V 중 최적의 미디어 선택. 자동 우선순위: I2V > T2V > 이미지.
6. CapCut 내보내기 — 원클릭으로 타임라인, 미디어, 자막, Ken Burns 애니메이션이 포함된 완성 프로젝트 내보내기.
...
5. 내보낼 미디어 선택 — 씬별로 이미지, T2V, I2V 중 선택. 자동 모드로 최적 미디어 자동 선택도 가능.
6. CapCut 내보내기 — CapCut 프로젝트를 생성. 바로 CapCut에서 열어 편집을 시작하세요.


⚡ 주요 기능

- Google Flow AI 통합 — 내장 브라우저로 앱 안에서 바로 Flow AI 접근. Chrome 확장 프로그램 불필요.
...
- 원클릭 CapCut 내보내기 — 타임라인, 미디어 파일, 자막, Ken Burns 애니메이션을 하나의 프로젝트 파일로.
- 스타일 프리셋 — 80종 이상의 내장 스타일 프리셋(애니메이션, 수채화, 시네마틱 등)으로 모든 씬에 일관된 비주얼 스타일 적용. '스타일 필수' 설정으로 생성 시 스타일 선택을 강제할 수 있습니다.
...
- 재생시간 자동 조정 — 비디오 클립 재생시간이 타임라인에서 자동 조정. 이미지 재생시간은 설정 가능.
- 자막 편집 — 씬 목록에서 바로 자막 편집. CSV/SRT에서 가져오기 가능.
- MCP 서버 (Claude Code 연동) — 내장 MCP 서버로 Claude Code에서 씬, 프롬프트, 레퍼런스를 직접 편집하고 생성을 트리거할 수 있습니다. AI 코딩 어시스턴트로 영상 제작 워크플로우 전체를 자동화하세요.
- 오픈 소스 — 영구 무료. GitHub에서 소스 코드 확인 및 기여 가능.


🤖 Claude Code 연동

AutoFlowCut에는 Claude Code와 직접 연결되는 내장 MCP(Model Context Protocol) 서버가 포함되어 있습니다. AI 코딩 어시스턴트로 영상 제작 워크플로우를 자동화할 수 있습니다:
...
- AI 스토리 채널 — 200개 이상의 씬에서도 캐릭터, 배경, 스타일의 일관성 유지.
- 숏폼 & TikTok 크리에이터 — AI 생성 장면을 빠르게 숏폼 영상 프로젝트로 변환.
- 교육 콘텐츠 제작자 — 대본을 AI 비주얼이 포함된 일러스트 영상 강의로 제작.
...
### Keywords (한국어)
```
AI 영상, CapCut, Google Flow, 텍스트투비디오, 이미지투비디오, AI 이미지 생성, 영상 자동화, 대량 생성, Ken Burns, 자막, SRT, 얼굴없는 유튜브, AI 스토리텔링, 오픈소스
```
...
100枚以上の画像生成。T2V・I2V動画生成。CapCutプロジェクトをワンクリックエクスポート。すべて1つのデスクトップアプリで。

AutoFlowCutはAI動画制作の全プロセスを自動化します。Google Flow AIで画像・動画を生成し、すぐに編集できるCapCutプロジェクトに変換。スクリプトをインポートし、ビジュアルを生成し、シーンごとに最適なメディアを選択して、ワンクリックでエクスポート。
...
- ワンクリックCapCutエクスポート — タイムライン、メディア、字幕、Ken Burnsアニメーション
- スタイルプリセット — 80種類以上の内蔵スタイルプリセット（アニメ、水彩画、シネマティックなど）で全シーンに一貫したビジュアルスタイルを適用。「スタイル必須」設定で生成時のスタイル選択を強制可能
- 自動タグマッチング — キャラクター、背景、スタイルの視覚的一貫性を維持
- MCPサーバー（Claude Code連携） — 内蔵MCPサーバーでClaude Codeからシーン、プロンプト、リファレンスを直接編集・生成トリガー可能
- 100%無料＆オープンソース

🤖 Claude Code連携
内蔵MCP（Model Context Protocol）サーバーにより、Claude Codeから動画制作ワークフローを自動化できます。プロンプトの一括編集、リファレンス管理、画像・動画生成のトリガー、カスタムスキルのインストールが可能です。

💰 無料＆オープンソース
すべての機能が無料。CapCutエクスポートのみ有料。GitHub: github.com/touchizen/AutoFlowCut
...
- ワンクリックCapCutエクスポート
- TXT / CSV / SRTインポート対応
- 80種類以上のスタイルプリセット（アニメ、水彩画、シネマティックなど）
- 内蔵MCPサーバーによるClaude Code連携
- カスタムスキルのインストールとワークフロー自動化
- HTTP API（ポート3210）で外部ツール連携
- 100%無料＆オープンソース
```
...
100+ Bilder generieren. T2V- und I2V-Videos erstellen. CapCut-Projekt mit einem Klick exportieren. Alles in einer Desktop-App.

AutoFlowCut automatisiert die gesamte KI-Videoproduktion — von der Google Flow AI Bild-/Videogenerierung bis zum fertigen CapCut-Projekt. Importieren Sie Ihr Skript, generieren Sie Visuals, wählen Sie das beste Medium pro Szene und exportieren Sie alles mit einem Klick.
...
- Ein-Klick CapCut-Export — Timeline, Medien, Untertitel und Ken Burns-Animationen
- Stil-Presets — Über 80 integrierte Stil-Presets (Anime, Aquarell, Cinematic usw.) für einheitliche visuelle Stile über alle Szenen. Optionale „Stil erforderlich"-Einstellung erzwingt die Stilauswahl vor der Generierung
- Automatisches Tag-Matching — Visuelle Konsistenz über alle Szenen
- MCP-Server (Claude Code) — Integrierter MCP-Server ermöglicht die direkte Bearbeitung von Szenen, Prompts und Referenzen über Claude Code
- 100% kostenlos & Open Source

🤖 Claude Code Integration
Der integrierte MCP-Server (Model Context Protocol) verbindet sich direkt mit Claude Code. Automatisieren Sie Ihren gesamten Videoproduktions-Workflow mit KI-Codierungsassistenten: Prompts bearbeiten, Referenzen verwalten, Generierung auslösen und benutzerdefinierte Skills installieren.

💰 Kostenlos & Open Source
Alle Funktionen kostenlos — nur CapCut-Export ist kostenpflichtig. GitHub: github.com/touchizen/AutoFlowCut
...
- Medienauswahl pro Szene
- Ein-Klick CapCut-Projektexport
- TXT / CSV / SRT Import
- 80+ Stil-Presets (Anime, Aquarell, Cinematic usw.)
- Integrierter MCP-Server für Claude Code Integration
- Installation benutzerdefinierter Skills und Workflow-Automatisierung
- HTTP API (Port 3210) für externe Tool-Anbindung
- 100% kostenlos & Open Source
```
